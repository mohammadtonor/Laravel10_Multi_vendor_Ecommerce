        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                return $editBtn = "<div class='d-flex'>
                    <a href='".route('admin.shipping-rules.edit', $query->id)."' class='btn btn-primary'><i class='far fa-edit'></i></a>
                    <a href='".route('admin.shipping-rules.destroy', $query->id)."' class='btn btn-danger ml-2 delete-item'><i class='far fa-trash-alt'></i></a>
                </div>";
            })
            ->addColumn('type', function ($query) {
                if($query->type == 'min_cost') {
                    return "<i class='badge badge-primary'>Minimum Order Amont</i>";
                } else {
                    return "<i class='badge badge-success'>Falt Amount</i>";
                }
            })
