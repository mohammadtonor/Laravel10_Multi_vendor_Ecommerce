                        ...
                        <div class="wsus__order_details" id="sticky_sidebar">
                            ...
                            @foreach ($shippingRule as $method)
                                @if ($method->type === 'min_cost' && getCartTotal() >= $method->min_cost)
                                    <div class="form-check">
                                        <input class="form-check-input shipping_method" data-id={{$method->cost}} type="radio" name="exampleRadios" id="exampleRadios1"
                                            value="{{$method->id}}" >
                            ...
                            <div class="wsus__order_details_summery">
                                ...
                                <p><b>total:</b> <span id="total_amount" data-id={{getMainCartTotal()}}><b>{{$settings->currency_icon}}{{getMainCartTotal()}}</b></span></p>
                            </div>
                            ...
                            <a href="" id="submitChechoutForm" class="common_btn">Place Order</a>
                        </div>
                    </div>
                </div>
        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body p-0">
                        <div class="wsus__check_form p-3">
                            <form method="POST" action="{{route('user.checkout.address.create')}}">
                                @csrf
                                <div class="row">
                                    <div class="col-md-12">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.shipping_method').on('click', function(e) {
                let shippngsFee = $(this).data('id');
                let currentTotalAmount = $('#total_amount').data('id');
                let totalAmount = currentTotalAmount + shippngsFee;

                $('#shipping_method_id').val($(this).val())
                $('#shipping_fee').text("{{$settings->currency_icon}}"+shippngsFee);
            ...
            $('#submitChechoutForm').on('click', function(e) {
                e.preventDefault();
                if($('#shipping_method_id').val() === '') {
                    ...
                    toastr.error('You have to agree website terms and condition')
                } else {
                    $.ajax({
                        ...
                        beforeSend: function () {
                            $('#submitChechoutForm').html("<i class='fas fa-spinner fa-spin fa-1x'></i>")
                        },
                        success: function(data){
                            if(data.status == 'success') {
                                $('#submitChechoutForm').text('Place Order');
